...
from goamapper.drawer import drawAreas, drawPaths, drawWays
...
class Generator:
    ...
    def _get_template(self) -> dw.Group:
        ...
        bg = dw.Rectangle(*self.canvas_dims, id="bg", fill=t.background_fill)
        ...
        for layer_name, layer_info in self.poster.map_layers.items():
            ...
            match layer_name:
                case "land":  # land is a background
                    map_content.append(
                        dw.Rectangle(id="land", *self.canvas_dims, fill=layer_info["fill"])
                    )

                case "water":  # water must be specially treated
                    map_content.append(
                        drawAreas(
                            fetcher.get_waterGDF(), id="water", fill=layer_info["fill"]
                        )
                    )
                ...
                case "streets":  # streets and other ways
                    ...
                    map_content.append(drawWays(gdf, layer_info, id=layer_name))

                # Removed f1 circuit path generation

                case _:
                    log.debug(f"Default case hit with {layer_name}")
                    gdf = fetcher.get_osmGDF(
    ...
    def create_map(self) -> dw.Drawing:
        ...
        groups = [self._get_map_content(), self._get_template(), self._get_text_area()]
